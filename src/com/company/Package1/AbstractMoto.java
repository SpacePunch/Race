...
public abstract class AbstractMoto implements Moto
{
	...
	public void race(AbstractMoto enemy)
	{
		int time1 = 0;
		int time2 = 0;
		for (int i = 0; i < 30000 / this.speed; i++) {
			time1++;
		}
		for (int i = 0; i < 30000 / enemy.speed; i++) {
			time2++;
		}
		time1 = time1 / 60;
		time2 = time2 / 60;
		...
		if (time1 > time2) {
			System.out.println("And the winner is " + enemy.name);
		}
		else {
			System.out.println("And the winner is " + this.name);
		}
		System.out.println();
	}
}
